...
def ingest_questions(pkg: dict, pages: int = 3):
    ...
    if all_questions:
        ...
        filepath, record_count = _save("questions", filename, envelope)
...
def ingest_answers(pkg: dict, question_ids: list,
    ...
    if all_answers:
        ...
        filename = f"{pypi}.json"
        filepath, record_count = _save("answers", filename, envelope)
...
def ingest_tag_info(pkg: dict):
    ...
    try:
        ...
        filename = f"{pypi}.json"
        filepath, record_count = _save("tag_info", filename, envelope)
